    ...
    <div className="text-xs text-[#4E5969]">
      ...
      {safeArtifacts.length === 0 ? (
        ...
        <div className="space-y-2">
          {safeArtifacts.map((a, i) => {
            const apiRef = api()
            const fileName = a.path?.split(/[/\\]/).pop() || '未知文件'
            return (
              <div key={i}
                onClick={() => apiRef?.file.open(a.path)}
                ...
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-[11px] font-medium text-[#1D2129] truncate group-hover:text-[#165DFF]">{fileName}</span>
                ...
                <div className="text-[10px] text-[#C9CDD4] mb-0.5 flex items-center gap-2">
                  <span>{a.time ? new Date(a.time).toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' }) : ''}</span>
